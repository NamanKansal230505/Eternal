...
import { updateNodeAlert, saveAlertToHistory } from '@/lib/firebase';
...
export function useNodeProcessor() {
  ...
  const processNodesWithAlerts = useCallback((nodesList: Node[]) => {
    ...
    nodesList.forEach(node => {
      ...
      // Save each alert to Firebase history for persistence
      nodeAlerts.forEach(alert => {
        saveAlertToHistory(alert).catch(err => 
          console.error(`Failed to save alert ${alert.id} to history:`, err)
        );
      });
