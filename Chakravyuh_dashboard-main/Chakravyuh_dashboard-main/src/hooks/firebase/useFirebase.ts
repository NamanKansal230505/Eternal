  ...
  useEffect(() => {
    ...
    try {
      unsubscribeNodes = subscribeToNodes((nodesList) => {
        ...
        // Always update alerts, even if activeAlertsList is empty, to preserve existing alerts
        setAlerts(prev => {
          ...
          // Keep alerts for up to 24 hours instead of just recent ones
          const now = Date.now();
          ...
          const combined = Array.from(existingAlertMap.values())
            .filter(alert => {
              ...
              return alertTime > twentyFourHoursAgo; // Keep alerts from last 24 hours
            })
            .sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime())
            // Limit to last 200 alerts to prevent memory issues
            .slice(0, 200); 
      ...
      unsubscribeAlerts = subscribeToAlerts((historyAlerts) => {
        // Merge historical alerts with current alerts
        setAlerts(prev => {
          ...
          // Add/update with current alerts (they take precedence)
          prev.forEach(alert => {
            alertMap.set(alert.id, alert);
          });
